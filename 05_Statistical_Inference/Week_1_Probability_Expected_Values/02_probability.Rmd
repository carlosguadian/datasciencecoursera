...
$$
where the $\{A_i\}$ are mutually exclusive. (Note a more general version of additivity is used in advanced classes.)
...
Then 
$P(A_1 \cup A_2) = P(A_1) + P(A_2) - P(A_1 \cap A_2) = 0.13 - \mbox{Probability of having both}$
...
- Discrete random variable are random variables that take on only a countable number of possibilities and we talk about the probability that they take specific values
  * $P(X = k)$
- Continuous random variable can take any value on the real line or some subset of the real line and we talk about the probability that they line within some range
  * $P(X \in A)$
...
A probability mass function evaluated at a value corresponds to the probability that a random variable takes that value. To be a valid pmf a function, $p$, must satisfy
  ...
  1. It must always be larger than or equal to 0. $p(x) \geq 0$ for all $x$
  2. The sum of the possible values that the random variable can take has to add up to one. $\sum_{x} p(x) = 1$
...
1. It must be larger than or equal to zero everywhere. $f(x) \geq 0$ for all $x$
